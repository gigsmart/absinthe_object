defmodule GreenFairy.ExtensionTest do
  ...
  defmodule TestExtension do
    ...
    def using(_opts) do
      quote do
        import GreenFairy.ExtensionTest.TestExtension.Macros
  ...
  describe "Extension behaviour" do
    test "using/1 callback is required" do
      ...
      assert {:using, 1} in GreenFairy.Extension.behaviour_info(:callbacks)
    end

    test "transform_field/2 callback is optional" do
      assert {:transform_field, 2} in GreenFairy.Extension.behaviour_info(:optional_callbacks)
    end

    test "before_compile/2 callback is optional" do
      assert {:before_compile, 2} in GreenFairy.Extension.behaviour_info(:optional_callbacks)
    end
  end

  describe "using extensions in types" do
    test "extension is registered in type definition" do
      definition = ExtendedUserType.__green_fairy_definition__()
      assert TestExtension in definition.extensions
    end

    test "__green_fairy_extensions__ returns list of extensions" do
      extensions = ExtendedUserType.__green_fairy_extensions__()
  ...
  describe "extension module detection" do
    ...
    test "regular modules are passed through unchanged" do
      ...
      # This is implicitly tested by the fact that GreenFairy.Type uses
