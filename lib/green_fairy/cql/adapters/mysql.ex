defmodule GreenFairy.CQL.Adapters.MySQL do
  @moduledoc """
  MySQL adapter for CQL operations.

  Delegates to scalar implementations with MySQL-specific behaviors:
  - ILIKE emulated via LOWER() LIKE LOWER()
  - Array operations via JSON functions
  - Limited NULLS positioning (none)
  """
  ...
  def operator_type_for(ecto_type) do
    ScalarMapper.operator_type_identifier(ecto_type, :mysql)
  end
  ...
  defp scalar_operator_input(ecto_type) do
    case ScalarMapper.operator_input(ecto_type, :mysql) do
  ...
  defp apply_operator_impl(query, field, operator, value, opts) do
    ...
    case ScalarMapper.scalar_for(field_type) do
      nil -> query
      scalar_module -> scalar_module.apply_operator(query, field, operator, value, :mysql, opts)
    end
  end
end
