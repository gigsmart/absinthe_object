defmodule Absinthe.Object.Schema do
  @moduledoc """
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @absinthe_object_namespaces unquote(namespaces)
      @absinthe_object_dataloader unquote(Macro.escape(dataloader_opts))
      @absinthe_object_query_module unquote(query_module)
      @absinthe_object_mutation_module unquote(mutation_module)
      @absinthe_object_subscription_module unquote(subscription_module)

      # For inline root definitions
      Module.register_attribute(__MODULE__, :absinthe_object_inline_query, accumulate: false)
      Module.register_attribute(__MODULE__, :absinthe_object_inline_mutation, accumulate: false)
      Module.register_attribute(__MODULE__, :absinthe_object_inline_subscription, accumulate: false)

      # Register our before_compile FIRST so it runs before Absinthe's
      @before_compile Absinthe.Object.Schema
      ...
      import Absinthe.Object.Schema, only: [root_query: 1, root_mutation: 1, root_subscription: 1]
    end
  end
  ...
  defp generate_using_query_block(module) do
    identifier = module.__absinthe_object_query_fields_identifier__()
  ...
  defp generate_using_mutation_block(module) do
    identifier = module.__absinthe_object_mutation_fields_identifier__()
  ...
  defp generate_using_subscription_block(module) do
    identifier = module.__absinthe_object_subscription_fields_identifier__()
  ...
  defmacro root_query(do: block) do
    quote do
      @absinthe_object_inline_query unquote(Macro.escape(block))
    end
  end
  ...
  defmacro root_mutation(do: block) do
    quote do
      @absinthe_object_inline_mutation unquote(Macro.escape(block))
    end
  end
  ...
  defmacro root_subscription(do: block) do
    quote do
      @absinthe_object_inline_subscription unquote(Macro.escape(block))
    end
  end

  @doc false
  defmacro __before_compile__(env) do
    namespaces = Module.get_attribute(env.module, :absinthe_object_namespaces)
    dataloader_opts = Module.get_attribute(env.module, :absinthe_object_dataloader)

    # Get explicit module configurations
    query_module = Module.get_attribute(env.module, :absinthe_object_query_module)
    mutation_module = Module.get_attribute(env.module, :absinthe_object_mutation_module)
    subscription_module = Module.get_attribute(env.module, :absinthe_object_subscription_module)

    # Get inline definitions
    inline_query = Module.get_attribute(env.module, :absinthe_object_inline_query)
    inline_mutation = Module.get_attribute(env.module, :absinthe_object_inline_mutation)
    inline_subscription = Module.get_attribute(env.module, :absinthe_object_inline_subscription)

    # Discover all type modules at compile time
    discovered = discover_at_compile_time(namespaces)
    grouped = Absinthe.Object.Discovery.group_by_kind(discovered)
    ...
    statements =
      [
        ...
        [
          quote do
            @doc false
            def __absinthe_object_discovered__ do
  ...
  ## Example
      ...
      def resolve_type(value, _) do
        Absinthe.Object.Schema.resolve_type_for(value, %{
          MyApp.User => :user,
          MyApp.Post => :post
        })
      end
